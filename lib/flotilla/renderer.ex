defmodule Flotilla.Renderer do
  ...
  defp render_node(%{node: {:text, opts, content}}) do
    ...
    ["<span", attrs, ">", escape_html(content), "</span>"]
  end
  ...
  defp render_node(%{node: {:heading, opts, content}}) do
    ...
    tag = "h#{level}"
    attrs = build_attrs(class, opts, [:class, :id])
    ["<", tag, attrs, ">", escape_html(content), "</", tag, ">"]
  end
  ...
  defp render_node(%{node: {:badge, opts, content}}) do
    ...
    ["<span class=\"", full_class, "\">", escape_html(content), "</span>"]
  end
  ...
  defp render_node(%{node: {:button, opts, label}}) do
    ...
    ["<button", attrs, ">", escape_html(label), "</button>"]
  end
  ...
  defp render_table_headers(columns) do
    Enum.map(columns, fn
      col when is_atom(col) ->
        ["<th>", escape_html(Atom.to_string(col)), "</th>"]
  ...
  defp cell_html(value) do
    ["<td>", escape_html(to_string(value)), "</td>"]
  end
  ...
  defp render_node_v2(%{node: {:divider, opts, nil}}) do
    ...
    cond do
      ...
      orientation == :vertical ->
        ["<div class=\"inline-block w-px h-full bg-gray-200 mx-2\"></div>"]
  ...
  defp render_node_v2(%{node: {:blockquote, opts, content}}) do
    ...
    ["<blockquote", attrs, ">", escape_html(content), "</blockquote>"]
  end
  ...
  defp render_node_v2(%{node: {:textarea, opts, content}}) do
    ...
    ["<textarea", attrs, ">", escape_html(content), "</textarea>"]
  end
  ...
  defp render_node_v2(%{node: {:submit, opts, label}}) do
    ...
    ["<button type=\"submit\"", attrs, ">", escape_html(label), "</button>"]
  end
  ...
  defp render_node_v2(%{node: {:stat, opts, %{label: label, value: value}}}) do
    ...
    trend_node =
      case trend do
        :up -> ["<span class=\"text-green-500 text-sm\">\u2191</span>"]
        :down -> ["<span class=\"text-red-500 text-sm\">\u2193</span>"]
        _ -> ""
      end
  ...
  defp render_node_v2(%{node: {:toast, opts, message}}) do
    ...
    full_class = "#{class} #{tone_class}" |> String.trim()
    ["<div class=\"", full_class, "\" role=\"status\">", escape_html(message), "</div>"]
  end
  ...
  defp render_node_v2(%{node: {:notification, opts, message}}) do
    ...
    ["<div class=\"", class, "\">", dot, escape_html(message), "</div>"]
  end
